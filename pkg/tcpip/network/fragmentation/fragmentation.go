...
// It is based on RFC 791, 815 and 8200.
...
// PacketFragmenterBase is the book-keeping struct for packet fragmentation.
type PacketFragmenterBase struct {
...
// NewPacketFragmenterBase prepares the struct needed for packet fragmentation.
//
...
func NewPacketFragmenterBase(pkt *stack.PacketBuffer, mtu uint32, extraHeaderLength int) PacketFragmenterBase {
	...
	// IPv6.
	// TODO(#3912): Once Authentication and/or ESP Headers are supported for
	// outbound packets, the length of the IPv6 fragmentable part need to take
	// these headers into account.
	fragmentablePartLength := pkt.TransportHeader().View().Size() + pkt.Data.Size()

	return PacketFragmenterBase{
...
// and returns it along with the fragment's offset, the number of bytes copied
// and a boolean indicating if there is more data left or not.
//
...
//
// Once there are no fragments left to be created, a nil PacketBuffer packet
// will be returned.
func (pf *PacketFragmenterBase) BuildNextFragment(proto tcpip.NetworkProtocolNumber) (*stack.PacketBuffer, uint16, uint16, bool) {
	...
	if n := len(pf.transportHeader); n > 0 {
		...
		if pf.currentFragment == 0 && pf.transportHeaderFitsFirst {
			copy(fragPkt.TransportHeader().Push(n), pf.transportHeader)
		} else {
